...
// Helper to fetch image and convert to Base64 for PDF/PPTX embedding
...
export const exportQuiz = async (quiz: Quiz, format: ExportFormat, options?: any): Promise<GeneratedFile> => {
  ...
  switch (format) {
    ...
    case ExportFormat.GOOGLE_SLIDES_PPTX: return await generatePresentation(quiz, sanitizedTitle);
...
// --- GOOGLE SLIDES / PPTX GENERATION ---
const generatePresentation = async (quiz: Quiz, title: string): Promise<GeneratedFile> => {
    // 1. Init PPTX
    const pptx = new PptxGenJS();
    pptx.layout = 'LAYOUT_16x9';
    pptx.author = 'Neural Quiz';
    pptx.company = 'Neural Quiz Converter';
    ...
    pptx.defineSlideMaster({
        ...
        objects: [
            ...
            { rect: { x: 0, y: 5.3, w: '100%', h: 0.3, fill: { color: 'F0F0F0' } } } // Bottom bar
        ]
    });

    // 2. Title Slide
    let slide = pptx.addSlide({ masterName: 'MASTER_SLIDE' });
    slide.addText(quiz.title, { x: 0.5, y: 2, w: 9, fontSize: 36, bold: true, align: 'center', color: '333333' });
    slide.addText(quiz.description || "Generated by Neural Quiz", { x: 0.5, y: 3.5, w: 9, fontSize: 18, align: 'center', color: '666666' });

    // 3. Question Loop
    for (let i = 0; i < quiz.questions.length; i++) {
        const q = quiz.questions[i];
        
        // We create TWO slides per question: 
        // 1. Question Only
        // 2. Question + Answer (Duplicate with highlight)
        
        const createSlide = async (showAnswer: boolean) => {
            const s = pptx.addSlide({ masterName: 'MASTER_SLIDE' });
            
            // Question Text
            s.addText(`${i + 1}. ${q.text}`, { 
                x: 0.5, y: 0.5, w: 9, h: 1.5, 
                fontSize: 24, bold: true, color: '333333', valign: 'top' 
            });
            ...
            if (q.imageUrl) {
                ...
                if (base64Img) {
                    s.addImage({ data: base64Img, x: 3.5, y: 1.5, w: 3, h: 2.5 });
                    currentY = 4.2;
                }
            }
            ...
            if (q.options.length > 0) {
                ...
                q.options.forEach((opt, idx) => {
                    ...
                    const yPos = currentY + (row * 1.2);
                    
                    // Determine style based on state
                    let boxColor = 'F9F9F9'; // Default gray
                    let textColor = '333333';
                    let borderColor = 'CCCCCC';

                    if (showAnswer) {
                        if (isCorrect) {
                            boxColor = 'D1FAE5'; // Light Green
                            borderColor = '10B981'; // Green
                            textColor = '065F46'; // Dark Green
                        } else {
                            boxColor = 'FFFFFF';
                            borderColor = 'E5E7EB';
                            textColor = '9CA3AF'; // Grayed out
                        }
                    }

                    // Draw Box
                    s.addShape(pptx.ShapeType.rect, { 
                        x: xPos, y: yPos, w: 4.3, h: 1, 
                        fill: { color: boxColor }, 
                    ...
                    s.addText(letter, {
                        x: xPos + 0.1, y: yPos + 0.1, w: 0.3, h: 0.3,
                        fontSize: 14, bold: true, color: '666666'
                    });

                    // Option Text
                    s.addText(opt.text, { 
                        x: xPos + 0.5, y: yPos + 0.1, w: 3.7, h: 0.8, 
                        fontSize: 14, color: textColor, valign: 'middle' 
                    });
                });
            } else if (showAnswer) {
                // Open Ended Answer Reveal
                s.addText("(Respuesta Abierta / Debate)", { x: 0.5, y: 3, w: 9, fontSize: 20, align: 'center', color: '666666', italic: true });
            }
        };

        // Generate Slide A (Question)
        await createSlide(false);
        // Generate Slide B (Answer)
        await createSlide(true);
    }

    const base64 = await pptx.write({ outputType: 'base64' });
    return {
        filename: `${title}_SLIDES.pptx`,
