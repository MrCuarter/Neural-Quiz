...
/**
 * Utility to ensure images are safe for Google APIs (Slides, Forms).
...
export const getSafeImageUrl = (originalUrl: string | undefined | null): string | null => {
    ...
    if (url.startsWith('data:') || url.startsWith('blob:')) {
        console.warn("[ImageProxy] Skipped Data/Blob URI (Not supported by Google Import APIs)");
        return null; 
    }
    ...
    // We force output=png to ensure compatibility with Google Slides/Forms which dislike WebP
    try {
