  ...
  const handleCreateAI = async () => {
    ...
    try {
      ...
      const enhancedQuestions = await Promise.all(generatedQs.map(async (gq) => {
          ...
          // CRITICAL UPDATE: Use imageSearchQuery (English) generated by AI
          // instead of raw text. This ensures stock photos are accurate.
          if (!qObj.imageUrl) {
              // Prioritize the dedicated English query from AI. Only fallback to text if missing.
              const query = qObj.imageSearchQuery || qObj.text;
              console.log('🤖 IA Smart Search:', query); // LOG REQUESTED
              const imageResult = await searchImage(query, qObj.fallback_category);
              
              if (imageResult) {
                  qObj.imageUrl = imageResult.url;
                  qObj.imageCredit = imageResult.credit; // STORE ATTRIBUTION
              }
          }
